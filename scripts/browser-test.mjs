...
try {
  ...
  await check('DynamicData cache refresh, paging, aggregates and HTML/React identity',async()=>{
    await goto('dynamicdata');
    await page.waitForFunction(()=>document.querySelectorAll('#dd-html-list li[data-item-id]').length===3);
    assert.deepEqual(await page.locator('#dd-html-list li[data-item-id]').evaluateAll(rows=>rows.map(row=>row.dataset.itemId)),['1','2','5']);
    assert.equal(await page.locator('#dd-total-hours').textContent(),'23');
    await page.evaluate(()=>{window.__dynamicDataRow=document.querySelector('#dd-html-list li[data-item-id="1"]');});
    await page.locator('#dd-promote').click();
    await page.waitForFunction(()=>document.querySelector('#dd-html-list li')?.getAttribute('data-item-id')==='3');
    assert.deepEqual(await page.locator('#dd-html-list li[data-item-id]').evaluateAll(rows=>rows.map(row=>row.dataset.itemId)),['3','1','2']);
    assert.equal(await page.locator('#dd-total-hours').textContent(),'33');
    assert.equal(await page.evaluate(()=>window.__dynamicDataRow===document.querySelector('#dd-html-list li[data-item-id="1"]')),true);
    await page.waitForFunction(()=>Array.from(document.querySelectorAll('#dd-react-list li[data-react-item-id]')).map(row=>row.getAttribute('data-react-item-id')).join(',')==='3,1,2');
    await page.locator('#dd-next').click();
    await page.waitForFunction(()=>document.querySelector('#dd-html-list li')?.getAttribute('data-item-id')!=='3');
    await page.locator('#dd-prev').click();
    await page.waitForFunction(()=>document.querySelector('#dd-html-list li')?.getAttribute('data-item-id')==='3');
    await page.locator('#dd-open-only').check();
    await page.locator('#dd-html-list li[data-item-id="3"] .dd-done').click();
    await page.waitForFunction(()=>!document.querySelector('#dd-html-list li[data-item-id="3"]'));
    await page.locator('#dd-open-only').uncheck();
    await page.locator('#dd-search').fill('Publish');
    await page.waitForFunction(()=>Array.from(document.querySelectorAll('#dd-html-list li[data-item-id]')).map(row=>row.getAttribute('data-item-id')).join(',')==='6');
    await page.locator('#dd-search').fill('');
    await page.locator('#dd-add').click();
    await page.locator('#dd-sort').selectOption('title');
    await page.waitForFunction(()=>document.querySelector('#dd-react-list li')?.getAttribute('data-react-item-id')==='7');
    await page.locator('#dd-react-list .dd-react-increase').first().click();
    await page.waitForFunction(()=>document.querySelector('#dd-total-hours')?.textContent==='40');
    assert.equal(await page.locator('#dd-html-list li[data-item-id="7"] .dd-hours').textContent(),'7 h ');
    assert.equal(await page.locator('#dd-source-count').textContent(),'7');
    await mkdir('test-results',{recursive:true});
    await page.screenshot({path:'test-results/showcase-dynamicdata.png',fullPage:true});
    await goto('overview');
    await page.evaluate(()=>{delete window.__dynamicDataRow;});
  });
