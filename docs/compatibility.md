...
| Derived lists | `BindableDerivedList` filtering, sorting and observed item refresh | **Adapted.** This implementation recomputes the projection; it is not the full incremental DynamicData operator engine. |
...
| Collection persistence | `AutoPersistCollection`, `ActOnEveryObject`, per-object save/cleanup | **Adapted.** Owns one resource per distinct live object. No arbitrary CLR metadata/serializer-attribute reader is provided. |
